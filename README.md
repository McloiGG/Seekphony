...
- Data component: persisted evaluation records with anonymous device hashes,
  file hashes, scores, metrics, weak segments, warnings, and Gemini explanation
  metadata.
...
- `SEEKPHONY_ADMIN_TOKEN`: optional backend-only token for global evaluation
  cleanup through the admin API. Never expose it to browser code.
...
3. Configure Render:
   ...
   - `SEEKPHONY_ADMIN_TOKEN=<private cleanup token>`
...
Evaluation and history routes require `X-Seekphony-Device-ID`, generated by the
frontend and stored in browser `localStorage`. The admin route requires
`X-Seekphony-Admin-Token` and is not called by the frontend.
...
- Persist structured per-browser evaluation history, inspect saved result
  details, delete one saved result, or clear the current browser's history.
...
- Anonymous device history stores a hash of a browser-generated UUID, separating
  normal guest histories without adding full authentication.
...
- Anonymous history is per browser profile and site storage, not verified user
